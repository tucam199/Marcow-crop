...
interface MartrendModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSelectImage: (imageUrl: string, textContext?: string, isAutoPost?: boolean) => void;
}
            ...
            <label className="flex items-center gap-3 mt-1 cursor-pointer">
              <div className="relative inline-block w-10 flex-shrink-0 align-middle select-none transition duration-200 ease-in">
                <input 
                  ...
                  style={{
                    right: isAutoPostEnabled ? '0' : '1.25rem',
                    borderColor: isAutoPostEnabled ? '#10B981' : '#E5E7EB',
                    background: isAutoPostEnabled ? '#10B981' : '#fff'
                  }}
                />
                <div className="toggle-label block overflow-hidden h-5 rounded-full bg-stone-200 cursor-pointer" style={{ backgroundColor: isAutoPostEnabled ? '#D1FAE5' : '#E5E7EB'}}></div>
              </div>
              <span className="text-sm font-medium text-stone-700">Tự động tạo & đăng FB</span>
            </label>
                    ...
                    <div key={idx} className="group relative rounded-xl overflow-hidden border border-stone-200 shadow-sm flex flex-col bg-white hover:shadow-md transition-shadow">
                      <div className="aspect-square w-full overflow-hidden relative bg-stone-100">
                        ...
                        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center p-4">
                          <button 
                            onClick={() => onSelectImage(post.imageUrl, post.text, isAutoPostEnabled)}
                          ...
                          >
                            {isAutoPostEnabled ? "Tạo & Đăng Ngay" : "Dùng tạo kịch bản"}
                          </button>
                        </div>
                      </div>
