...
async function startServer() {
  ...
  // ===== GET CONFIG (returns API key to authenticated users) =====
  app.post("/api/get-config", (req, res) => {
    ...
    try {
      ...
      const payload = Buffer.from(payloadBase64, "base64").toString("utf-8");
      const expected = crypto.createHmac("sha256", secret).update(payload).digest("hex");
      if (signature !== expected) return res.status(401).json({ error: "Invalid token" });

      const data = JSON.parse(payload);
      const maxAge = 7 * 24 * 60 * 60 * 1000;
      if (Date.now() - data.ts > maxAge) return res.status(401).json({ error: "Token expired" });
